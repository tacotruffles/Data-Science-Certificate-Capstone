...
```
After **NA's** are removed, there are `666` ablums from `231` artists, with an average of `2.9` releases each. In the raw data there were `865` albums with just over `350` artists averaging `3.3` albums. Not much of a shift in the number of releases, which further shows that most of my music from 1980 onward is Rock and Pop.
